...
# Uses HTTPS API — works on all hosting platforms including Railway free tier
...
def _send_email(to_email: str, subject: str, html_body: str, text_body: str) -> bool:
    """Core send function using Resend API via urllib (no extra dependencies)."""
    ...
    try:
        ...
        logger.error("Resend HTTP error %s for %s: %s", e.code, to_email, body)
        return False
...
def send_otp_email(to_email: str, otp_code: str, full_name: str) -> bool:
    ...
    text_body = (
        f"Hi {full_name},\n\n"
        f"Your JobPortal verification code is: {otp_code}\n\n"
        f"This code expires in 10 minutes.\n\n"
        f"If you didn't create a JobPortal account, ignore this email."
    )
