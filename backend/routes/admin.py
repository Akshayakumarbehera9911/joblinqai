...
def update_user(
    ...
    if user.role == "admin" and admin.id != user.id:
...
def delete_company(
    ...
    company = db.query(Company).filter(Company.id == company_id).first()
    if not company: raise HTTPException(404, "Company not found")
    # Close all jobs from this company
...
def update_report(
    ...
    if body.status not in ("reviewed", "resolved", "dismissed"):
        raise HTTPException(400, "Invalid status")
...
# ── POST /api/admin/reports ─ anyone logged in can submit a report ─────────
class ReportCreate(BaseModel):
    report_type: str          # scam_job / fake_company / candidate_fraud / bug / other
