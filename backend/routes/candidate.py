...
def get_profile(current_user: User = Depends(require_candidate), db: Session = Depends(get_db)):
    ...
    return {
        ...
        "data": {
            "profile": {
                "id": profile.id, "user_id": profile.user_id, "photo_url": profile.photo_url,
                "gender": profile.gender,
                "date_of_birth": str(profile.date_of_birth) if profile.date_of_birth else None,
                "state": profile.state, "district": profile.district, "city": profile.city,
                "pincode": profile.pincode, "education_level": profile.education_level,
                "education_field": profile.education_field, "institution": profile.institution,
                "passing_year": profile.passing_year, "experience": profile.experience,
                "current_title": profile.current_title, "industry": profile.industry,
                "resume_url": profile.resume_url, "target_role": profile.target_role,
                "job_type": profile.job_type, "work_mode": profile.work_mode,
...
@router.post("/resume")
async def upload_resume(file: UploadFile = File(...), current_user: User = Depends(require_candidate), db: Session = Depends(get_db)):
    profile = get_profile_or_404(current_user.id, db)
    ...
    try:
        ...
        pass
    profile.resume_url = url; profile.resume_text = resume_text.strip()
    db.commit()
    return {"success": True, "data": {"resume_url": url, "text_extracted": bool(resume_text)}, "error": None}
...
def delete_resume(current_user: User = Depends(require_candidate), db: Session = Depends(get_db)):
    ...
    try:
        ...
        if public_id: delete_file(public_id, resource_type="raw")
    except Exception:
        pass
    profile.resume_url = None; profile.resume_text = None
    db.commit()
...
def get_skills(current_user: User = Depends(require_candidate), db: Session = Depends(get_db)):
    profile = get_profile_or_404(current_user.id, db)
    skills = db.query(CandidateSkill).filter(CandidateSkill.candidate_id == profile.id).all()
    return {"success": True, "data": [{"id": s.id, "skill_name": s.skill_name, "category": s.category, "level": s.level} for s in skills], "error": None}
...
def delete_skill(skill_id: int, current_user: User = Depends(require_candidate), db: Session = Depends(get_db)):
    profile = get_profile_or_404(current_user.id, db)
    skill = db.query(CandidateSkill).filter(CandidateSkill.id == skill_id, CandidateSkill.candidate_id == profile.id).first()
    if not skill:
...
# ── GET/POST/DELETE /api/candidate/certifications ─────────────────────────────
@router.get("/certifications")
def get_certs(current_user: User = Depends(require_candidate), db: Session = Depends(get_db)):
    profile = get_profile_or_404(current_user.id, db)
    certs = db.query(CandidateCertification).filter(CandidateCertification.candidate_id == profile.id).all()
    return {"success": True, "data": [{"id": c.id, "cert_name": c.cert_name, "platform": c.platform, "year_completed": c.year_completed, "cert_url": c.cert_url} for c in certs], "error": None}
...
def delete_cert(cert_id: int, current_user: User = Depends(require_candidate), db: Session = Depends(get_db)):
    profile = get_profile_or_404(current_user.id, db)
    cert = db.query(CandidateCertification).filter(CandidateCertification.id == cert_id, CandidateCertification.candidate_id == profile.id).first()
    if not cert:
...
# ── GET/POST/DELETE /api/candidate/projects ───────────────────────────────────
@router.get("/projects")
def get_projects(current_user: User = Depends(require_candidate), db: Session = Depends(get_db)):
    profile = get_profile_or_404(current_user.id, db)
    projects = db.query(CandidateProject).filter(CandidateProject.candidate_id == profile.id).all()
    return {"success": True, "data": [{"id": p.id, "title": p.title, "description": p.description, "tech_stack": p.tech_stack, "project_url": p.project_url, "year": p.year} for p in projects], "error": None}
...
def delete_project(project_id: int, current_user: User = Depends(require_candidate), db: Session = Depends(get_db)):
    profile = get_profile_or_404(current_user.id, db)
    project = db.query(CandidateProject).filter(CandidateProject.id == project_id, CandidateProject.candidate_id == profile.id).first()
    if not project:
...
@router.post("/photo")
async def upload_photo(file: UploadFile = File(...), current_user: User = Depends(require_candidate), db: Session = Depends(get_db)):
    profile = get_profile_or_404(current_user.id, db)
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Upload failed: {str(e)}")
    profile.photo_url = url; db.commit()
...
def dashboard(current_user: User = Depends(require_candidate), db: Session = Depends(get_db)):
    ...
    profile = db.query(CandidateProfile).filter(CandidateProfile.user_id == current_user.id).first()
    if not profile:
        return {"success": True, "data": {"profile_complete": False, "score": None, "gaps": [], "top_jobs": [], "message": "Please complete your profile to see your readiness score."}, "error": None}

    score    = db.query(ReadinessScore).filter(ReadinessScore.candidate_id == profile.id, ReadinessScore.target_role == profile.target_role).first()
    ...
    return {
        ...
        "data": {
            "profile_complete": profile.profile_complete,
            "full_name": current_user.full_name, "photo_url": profile.photo_url,
            "target_role": profile.target_role,
            "score": {"overall": score.overall_score, "experience": score.experience_score, "projects": score.project_score, "skills": score.skills_score, "education": score.education_score, "breakdown": score.score_breakdown, "ml_verdict": score.ml_verdict} if score else None,
            "gaps": [{"skill_name": g.skill_name, "gap_level": g.gap_level, "udemy_link": g.udemy_link} for g in gaps],
            "top_jobs": top_jobs,
...
def get_my_applications(current_user: User = Depends(require_candidate), db: Session = Depends(get_db)):
    ...
    cand_skills = [s.skill_name for s in db.query(CandidateSkill).filter(CandidateSkill.candidate_id == profile.id).all()]
    ...
    for a in apps:
        ...
        if match_score is None:
            ...
            ranked = rank_jobs_for_candidate(cand_skills, [{"job": job, "mandatory_skills": mandatory, "optional_skills": optional}])
            if ranked: match_score = ranked[0]["match_score"]
        result.append({
            "application_id": a.id, "job_id": job.id, "job_title": job.title,
            "company_name": company.company_name if company else "Unknown",
            "city": job.city, "work_mode": job.work_mode, "job_type": job.job_type,
            "salary_min": job.salary_min, "salary_max": job.salary_max,
            "status": a.status, "match_score": match_score,
...
def withdraw_application(application_id: int, current_user: User = Depends(require_candidate), db: Session = Depends(get_db)):
    from backend.models.application import Application
    ...
    if app.status != "applied":
        raise HTTPException(status_code=400, detail=f"Cannot withdraw — application is already '{app.status}'")

    app.status = "withdrawn"
    db.commit()
    return {"success": True, "data": {"message": "Application withdrawn"}, "error": None}


# ── POST /api/candidate/photo/remove ─────────────────────────────────────────
...
def remove_photo(current_user: User = Depends(require_candidate), db: Session = Depends(get_db)):
    ...
    profile.photo_url = None; db.commit()
